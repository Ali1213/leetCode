...
/**
 * Runtime: 104 ms, faster than 88.82% of JavaScript online submissions for Heaters.
 * Memory Usage: 39.1 MB, less than 66.67% of JavaScript online submissions for Heaters.
 * @param {number[]} houses
...
const findRadius = function (houses, heaters) {
    ...
    for (let i = 0; i < houses.length; i++) {
        while (j < heaters.length - 1 && Math.abs(houses[i] - heaters[j]) >= Math.abs(houses[i] - heaters[j + 1])) j++;
...
test.forEach(([A, B, r], index) => {
    try {
        assert.deepEqual(findRadius(A, B), r);
    } catch (e) {
